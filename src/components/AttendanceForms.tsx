...
const AttendanceForms = () => {
  ...
  const isClassComplete = (classData: ClassData): boolean => {
    return Boolean(
      classData.teacher && classData.student.every((student) => student.status)
    );
  };
  ...
  const handleSubmit = (classId: number) => {
    setClassAttendance((prev) =>
      prev.map((classData) =>
        classData.id === classId ? { ...classData, submitted: true } : classData
      )
    );
              ...
              <TableRow key={`${classData.id}-${student.id}`}>
                {studentIndex === 0 && (
                  <>
                    <TableCell
                      ...
                      rowSpan={classData.student.length}
                    >
                      {classData.time}
                    </TableCell>
                    <TableCell
                    ...
                    >
                      <Select
                        ...
                        onValueChange={(value) =>
                          handleTeacherChange(classData.id, value)
                        }
                ...
                <TableCell className="border-r-2">
                  <div className="flex gap-2">
                    {(["Attended", "Absent", "Freeze"] as const).map(
                      (status) => (
                        <Button
                          key={status}
                          size="sm"
                          className={`px-3 py-1 text-sm ${getStatusButtonStyle(
                            student.status,
                            status
                          )}`}
                          onClick={() =>
                            handleStatusChange(classData.id, student.id, status)
                          }
                          disabled={classData.submitted}
                        >
                          {status}
