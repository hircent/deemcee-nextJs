...
const formSchema = z.object({
  ...
  business_reg_no: z.string().min(1, 'Business registration number is required'),
  address: z.object({
      ...
      <DialogContent className="w-full max-w-[1000px] h-[90vh] overflow-y-auto custom-scrollbar bg-white">
        <DialogHeader>
          <DialogTitle className="text-xl sm:text-2xl font-semibold">
            {isLoading ? 'Loading Branch Details' : 'Edit Branch'}
          </DialogTitle>
          {!isLoading && (
            <DialogDescription className="text-sm sm:text-base">
              Make changes to your branch details here. Click save when you're done.
            </DialogDescription>
          )}
        </DialogHeader>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <DialogFooter className="gap-4 sm:gap-0">
                <Button type="submit" className="bg-[#000] text-white" disabled={isLoading}>
