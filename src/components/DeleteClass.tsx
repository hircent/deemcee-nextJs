...
import { DeleteClassProps } from "@/types/index";
...
export function DeleteClass({ type, name, id }: DeleteClassProps) {
  const [open, setOpen] = useState<boolean>(false);
  const [zoderror, setZodError] = useState<ClassFormErrors | null>(null);
  const formRef = useRef<HTMLFormElement>(null);
  const { toast } = useToast();
  const [state, formAction] = useFormState(deleteClass, SERVER_ACTION_STATE);
