...
import { DeleteCalendarProps } from "@/types/index";
import { Trash2 } from "lucide-react";
import { useEffect, useRef, useState } from "react";
import { useToast } from "./ui/use-toast";
import { SERVER_ACTION_STATE } from "@/constants/index";
import { useFormState } from "react-dom";
import { deleteCalendar } from "@/lib/actions/calendar.action";
import { HolidayEventError } from "@/types/calendar";
import { cn } from "@/lib/utils";

export function DeleteCalendar({ type, name, id }: DeleteCalendarProps) {
  const [open, setOpen] = useState<boolean>(false);
  const [zoderror, setZodError] = useState<HolidayEventError | null>(null);
  const formRef = useRef<HTMLFormElement>(null);
  const { toast } = useToast();
  const [state,formAction] = useFormState(deleteCalendar,SERVER_ACTION_STATE)
